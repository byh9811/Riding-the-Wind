      ...
      <div class='row'>
        <div class='col-md-3' id='left-container'>
          ...
          <div class="list-group">
            ...
            <a href="/user/changepwd" class="list-group-item list-group-item-action">비밀번호 변경</a>
    	...
    	<c:forEach var="personalTrip" items="${personalTrips}">
    		<div draggable="true" class="card col" style="width: 13rem;">
              <img src="${personalTrip.firstImage}" class="card-img-top" alt="areaImage" onerror=this.src="${root}/asset/img/noimg.jpg">
              <div class="card-body">
                <h6 class="card-title">${personalTrip.title}</h6>
                <p class="card-text">${personalTrip.addr1}</p>
                <a href="#" class="btn btn-primary" onclick="moveCenter(${personalTrip.latitude}, ${personalTrip.longitude});">위치</a>
              	<div style="display: none" class="latitude">${personalTrip.latitude}</div>
	        	<div style="display: none" class="longitude">${personalTrip.longitude}</div>
                <a href="mypage?action=deleteTrip&contentId=${personalTrip.contentId}" class="btn btn-danger">삭제</a>
              </div>
            </div>
    	</c:forEach>
          </div>
